...
state_nameids = [
    state_name_to_nameid(name) for name in state_name_abbrev_map.values()
]
...
def fit_vax_scaledhill_from_endpoint(t_obs, y_obs, t_end, y_t_end):
    ...
    def vax_scaledhill_objective(par):
        ...
        if not np.isfinite(scale) or scale <= 0.0 or n <= 0.0 or t_h <= 0.0:
            ...
            y_pred = vax_scaledhill_with_endpoint(
                t_obs, n, t_h, t_end, y_t_end
            )
    ...
    a0 = 0.99
    n0 = 1.0
    t_h_0 = (t_end**n0 * (a0 / y_t_end - 1)) ** (
        1 / n0
    )  # solved from initial scale a0, shape n0
...
def plot_timeseries(
    t_obs, y_obs, t_end, y_t_end, scenario_dir, state_nameid, age
):
    """
    ...
    # Save plot
    plt.savefig(
        os.path.join(scenario_dir, f"{state_nameid}-{age}-timeseries.pdf"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def plot_fit(
    ...
    # Save the plot
    plt.savefig(
        os.path.join(scenario_dir, f"{state_nameid}-{age}-fit.pdf"),
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def process_vax_data(df: pd.DataFrame):
    ...
    df_filtered["state"] = pd.Categorical(
        df_filtered["state"], categories=state_nameids, ordered=True
    )
    # Sort by state, age, and date (equivalent to arrange(state, age, date))
    df_processed = df_filtered.sort_values(
        ["state", "age", "date"]
...
def format_vax_2324(df: pd.DataFrame):
    ...
    df = df.query('Risk_group == "Overall"').rename(
        columns={"Age": "age", "Date": "date"}
    )
...
def write_json(state_fit_results, scenario_dir, state_nameid):
    ...
    with open(
        os.path.join(scenario_dir, f"{state_nameid}_hillvax_config.json"), "w"
    ) as f:
...
def fit_scenario_state_age(
    ...
    assert (
        len(vax_df["state"].unique()) == 1
    ), "Data should contain only one state"
    assert all(
        vax_df["state"] == state_nameid
    ), "All data should be for the specified state"
    assert (
        len(vax_df["age"].unique()) == 1
    ), "Data should contain only one age group"
    assert all(
        vax_df["age"] == age
    ), "All data should be for the specified age group"
    ...
    print(
        f"    fitting {state_nameid}, {age} (ratio 23-24 {final_frac_vax_state_2425:.6f}, t_end {t_end}, final frac {final_frac_vax_state_2425:.6f})"
    )
    ...
    plot_fit(
        ...
        final_frac_vax,
        fit_result,
        y_prev=frac_prev_obs,
        scenario_dir=scenario_dir,
        state_nameid=state_nameid,
        age=age,
    )
    ...
    if not fit_result["success"]:
        print(
            f"Warning: Optimization did not converge for {state_nameid}, {age}"
        )
...
def fit_scenario(
    ...
    with open(
        os.path.join(scenario_dir, "_allstates_hillvax_config.json"), "w"
    ) as f:
        ...
        if "fits" in state_fit:
            ...
            for i, age in enumerate(age_factor_levels):
                if i < len(fits):
                    fit_data = fits[i]
                    if (
                        fit_data.get("scale", 0) > 1
                        or fit_data.get("t_halfsat", 0) > 1000
                    ):
                        is_problematic = True
                        break
    ...
    if problematic_results:
        with open(
            os.path.join(scenario_dir, "_badstates_hillvax_config.json"), "w"
        ) as f:
            json.dump(problematic_results, f, indent=2, default=str)

        print(
            f"Found {len(problematic_results)} problematic states with unusual fits"
        )
...
def fit_scenario_state(
    ...
    for i, age_i in enumerate(age_factor_levels):
        ...
        final_frac = (
            min(final_state_frac_vax[i] * 1.01, 0.98)
            if i < len(final_state_frac_vax)
            else 0.5
        )
    ...
    return {"state": state_nameid, "fits": state_fit_results}
...
def add_vax_2324(vax_df: pd.DataFrame, covid_vax_2324: pd.DataFrame):
    ...
    combined_df = vax_df.merge(
        covid_vax_2324, how="left", on=["state", "age", "date"]
    )
    return combined_df
