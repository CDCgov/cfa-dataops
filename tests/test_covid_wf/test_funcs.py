...
from cfa.scenarios.dataops.workflows.covid.funcs.process_data import (
    total_cases_using_hospital_and_sero, cases_by_variant,
    individual_infection_totals,
)
from cfa.scenarios.dataops.workflows.covid.funcs.tslie import get_tslie
from cfa.scenarios.dataops.workflows.covid.funcs.check import check_blob_date_exists
...
def test_check_blob_date_exists_today(monkeypatch):
    today = "2024-06-11"
    with mock.patch("cfa.scenarios.dataops.workflows.covid.funcs.check.datetime") as mock_datetime:
...
def test_import_vaccination_data_blob():
    ...
    with patch("cfa.scenarios.dataops.workflows.covid.funcs.import_data.get_today_date", return_value="2024-06-02"):
        result = import_vaccination_data("2024-06-02", "unused_path/", blob_ep=mock_blob)
    assert isinstance(result, pd.DataFrame)
    assert all(result["date"] <= "2024-06-02")
...
def test_import_population_data_basic():
    ...
    with patch("cfa.scenarios.dataops.workflows.covid.funcs.import_data.get_data") as mock_get_data:
...
def test_import_variant_data_filters_and_returns():
    ...
    assert set(["date", "variant", "proportion", "region"]).issubset(result.columns)
...
def test_import_hospitalization_data_local(tmp_path):
    ...
    state = pd.Series({"stname": "California"})
    # Should only return rows with date <= start
    result = import_hospitalization_data("2024-06-02", str(tmp_path) + "/", state)
    assert isinstance(result, pd.DataFrame)
    assert all(result["date"] <= "2024-06-02")
...
def test_import_hospitalization_data_blob():
    ...
    state = pd.Series({"stname": "California"})
    with patch("cfa.scenarios.dataops.workflows.covid.funcs.import_data.get_today_date", return_value="2024-06-02"):
        result = import_hospitalization_data("2024-06-02", "unused_path/", state, blob_ep=mock_blob)
    assert isinstance(result, pd.DataFrame)
    assert all(result["date"] <= "2024-06-02")
    assert set(result.columns) == {"date", "state", "hospital_weekly"}
...
def test_format_vac_data_basic():
    ...
    assert set(["date", "age", "dose", "percent", "total"]).issubset(vac_history.columns)
    assert set(["date", "age", "dose", "percent", "total"]).issubset(vac.columns)
...
def test_output_generation_basic():
    ...
    row = output.iloc[0]
    inf = infections[(infections.age == row["age"]) & (infections.type == row["type"])]["percent"].values[0]
    vax = vac[(vac.age == row["age"]) & (vac.dose == row["dose"])]["percent"].values[0]
...
def test_cases_by_variant_basic():
    ...
    for _, row in result.iterrows():
        ...
        vprop = variant[(variant["date"] == row["date"]) & (variant["variant"] == row["strain"])]["proportion"]
        # Find original cases for this age/date
        orig_cases = cases[(cases["date"] == row["date"]) & (cases["age"] == row["age"])]["cases"]
...
def test_individual_infection_totals_single_variant():
    ...
    for age in pop["age"]:
        ...
        assert omicron_row["total"] == infect[infect["age"] == age]["cases"].values[0]
        assert none_row["total"] == pop[pop["age"] == age]["census"].values[0] - omicron_row["total"]
...
def test_get_tslie2():
    ...
    assert set(tslie2.columns).issubset(set(['age', 'dose', 'type', 'percent', 'tslie_group_0', 'tslie_group_1', 'tslie_group_2', 'tslie_group_3', 'tslie_group_4']))
